...
- **270+ Unicode styles** across 16 categories — bold, italic, cursive, bubble, squared, gothic, fraktur, decorative, upside-down, reverse, morse, glitch, zalgo and more.
- **20+ platform & generator pages** — Instagram, Facebook, X, TikTok, WhatsApp, Discord, Snapchat, Telegram, LinkedIn, YouTube, Twitch, Free Fire, PUBG, Roblox, Fortnite, Minecraft, Mobile Legends, COD Mobile, Valorant, Gaming, Username Studio, and Kaomoji Explorer.
- **Data-driven Apps Dropdown (Mega Menu)** — 4-column scrollable app launcher powered by `lib/platform/navigation.ts`.
- **Universal Search & Shortcut** — Instant global tool lookup triggered visually or via `Cmd+K` / `Ctrl+K`.
- **Premium Glassmorphism UI System** — Opaque frosted-glass surfaces (`.glass-card`, `.glass-panel`) with zero backdrop distortion.
...
- **Symbol library** — thousands of symbols across categories (hearts, stars, gaming glyphs, royal, brackets, arrows and kawaii symbols).
- **Kaomoji library** — searchable japanese text emoticons with category filters and instant copy.
- **Username Studio** — theme-based username generation with platform availability scoring.
- **Gaming name composer** — prefixes, clan tags and suffixes for styled gamer names.
- **Favorites & recent** — pin your go-to styles and quickly re-copy recent ones.
...
components/
  app/               # App experience & dedicated generator layout components
  filters/           # Category, family, and input filter chips
  generator/         # Core generator, StyleGrid, StyleCard, SymbolLibrary, ComparisonTray, GamingComposer
  home/              # Home experience, Explorer, stage sections
  kaomoji/           # KaomojiCard, KaomojiGrid, KaomojiExplorer, KaomojiComposer, KaomojiModal
  layout/            # Header, Footer, GlyphtiqNav, AppsDropdown, NavPill, MobileMenu
  platform/          # UniversalSearchBar and search triggers
  symbols/           # SymbolsExplorer, SymbolCard, SymbolFilters, SymbolModal
  ui/                # Button, Toast, AnimatedList, FloatingLines, ClickSpark
  usernames/         # UsernameStudio, UsernameCard, UsernameShelf
lib/
  text-engine/       # Alphabets, decorators, styles, per-app configs & limits
  kaomoji/           # Kaomoji dataset and search logic
  symbols/           # Unicode symbol data & categories
  usernames/         # Username generation rules and scoring
  platform/          # Data-driven navigation config (NAV_SECTIONS)
